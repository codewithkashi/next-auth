...
<div className="grid gap-x-4 gap-y-4 w-full mt-8 mb-12 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
  {Object.entries(manifest.adapters).map(([value, label]) => (
    <Link
      href={`/getting-started/adapters/${value}`}
      key={value}
      className="p-4 border border-solid border-neutral-200 dark:border-neutral-800 rounded-lg flex flex-col items-center justify-between w-36 shadow-sm h-32 hover:bg-neutral-50 dark:hover:bg-neutral-950 transition duration-300"
    >
