...
Auth.js has a conecpt of database adapters which lets you save and retrieve data from a database/service related to users, accounts, sessions, and verification tokens, passkeys and more.

Auth.js is little opinionated when it comes to persisting data. It is compatible with any database/service, and even supports mixing and matching them - for example using Redis for sessions and Email logins, and an SQL database for persisting user and account data.

You also only need to create the database tables/columns that are actually going to be used.
...
All our official database adapters are available under the `@auth/` scope on npm. If we don't have an [official package](https://github.com/nextauthjs/next-auth/tree/main/packages) available yet, you can create your own! (We welcome PRs for new adapters! See the [guidelines](#official-adapter-guidelines) below.)

<Callout type="info">
  _By default_, Auth.js saves values that are coming from JavaScript as
  `camelCase`, while respecting the conventional `snake_case` formatting for
  OAuth-related values. However, this is not a strict requirement. Most built-in
  adapters we provide will support a configuration option to change the table
  and column names, and you map the values however you like in your own adapter.
</Callout>
