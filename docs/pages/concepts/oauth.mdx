...
sequenceDiagram
    ...
    participant GitHub as "Auth Server (GitHub)"
    ...
    App Server->>GitHub: GET<br/>"github.com/login/oauth/authorize"
    Note left of GitHub: Sign in options<br> are supplied as<br/>query params<br/>(clientId, <br/>scope, etc...)
    GitHub->>Browser: Shows sign in page<br/>in GitHub.com<br/>to the user
    Note left of Browser: User inserts their<br/>credentials in GitHub
    Browser->>GitHub: GitHub validates the inserted credentials
    GitHub->>GitHub: Generates one time access code<br/>and calls callback<br>URL defined in<br/>App settings
    GitHub->>App Server: GET<br/>"api/auth/github/callback?code=123"
    App Server->>App Server: Grabs code<br/>to exchange it for<br/>access token
    App Server->>GitHub: POST<br/>"github.com/login/oauth/access_token"<br/>{code: 123}
    GitHub->>GitHub: Verifies code is<br/>valid and generates<br/>access token
    GitHub->>App Server: { access_token: 16C7x... }
